...
import { Resident } from '@/app/[locale]/(user)/(country-city)/components/resident';
...
export default function Residents({ params }: { params: { slug: string } }) {
  ...
  const { data, fetchNextPage, hasNextPage } = useInfiniteQuery<User[]>({
    queryKey: ['country', 'residents', params.slug],
  ...
  return (
    <div>
      ...
      <div>
        ...
        <div className="grid divide-y-2 gap-x-2 grid-cols-1 sm:grid-cols-2">
